...
public class EntityEventHandler {
  ...
  public static void onEntityDamage (LivingDamageEvent.Post event) {
    ...
    Player player = null;
    if (event.getEntity().getType().is(TFLootrTags.Entity.PLAYERS)) {
      var source = event.getSource().getEntity();
      if (event.getEntity() instanceof Player player2) {
        player = player2;
      }
      if (source != null && source.getType().is(TFLootrTags.Entity.BOSSES) && source instanceof IHasBossTracking) {
    ...
    } else if (event.getEntity().getType().is(TFLootrTags.Entity.BOSSES) && event.getEntity() instanceof
    IHasBossTracking tracking) {
      var source = event.getSource().getEntity();
      if (source != null && source.getType().is(TFLootrTags.Entity.PLAYERS) && source instanceof Player player2) {
